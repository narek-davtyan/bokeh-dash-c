...
# Load data on server
df_orig = pd.read_excel(r'bokeh-dash-c/result_data_x.xlsx', names=['index', 'type', 'date', 'code', \
    'filter_one', 'filter_two', 'filter_three', 'filter_four', 'recommendation', 'easiness', 'overall', 'question_one', \
    'rec_sc', 'eas_sc', 'sentiment', 'lang', 'question_one_filtered_lemmas'])
...
for type_filter in general_dict['type_list']:
    ...
    general_dict['individual_filters'][type_filter] = np.concatenate((df_orig.filter_one.unique(),df_orig.filter_four.unique())).tolist() + [type_filter]
...
def update_filters():
    ...
    common_filters = general_dict['filter_1'].value + general_dict['filter_4'].value
    for type_filter in general_dict['type_list']:
...
def filter_df(type_filter):
    ...
    general_dict['individual_filtered_df'][type_filter] = general_dict['individual_df'][type_filter].loc[(general_dict['individual_df'][type_filter]['filter_one'].isin(filter_list) & general_dict['individual_df'][type_filter]['filter_four'].isin(filter_list))].copy()
...
data_source = ColumnDataSource(pd.DataFrame(columns=['filter_one', 'filter_four', 'recommendation', 'easiness']))
...
def typed_callback_c(type_filter):
    ...
    current = df_points.loc[(df_points['recommendation'].isin(recommendations)) & (df_points['easiness'].isin(easinesses)) & (df_points['filter_one'].isin(general_dict['individual_filters'][type_filter]))  & (df_points['filter_four'].isin(general_dict['individual_filters'][type_filter]))]
...
buttons_row = row(column(children=[general_dict['filter_1'], button_1, button_2], height=200, max_height=200), column(children=[general_dict['filter_4'], button_3, button_4], height=200, max_height=200, width=320), data_table)
...
def init_frequency_points(type_filter):
    ...
    filter_columns = ['filter_one', 'filter_four']
    ...
    # print('freq_df:::',general_dict['freq_df'][type_filter])
